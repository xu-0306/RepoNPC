...
- Connection writes accept `endpoint_action: replace|retain` (default `replace` for existing clients). Creation requires `replace` and a valid `base_url`. Ordinary managed updates with `retain` require an omitted/null URL and use the protected current endpoint without returning it; `replace` requires a new URL. A host-managed update rejects `retain` as `HOST_CONNECTION_REPLACEMENT_REQUIRED`, accepts only a complete newly entered URL, and converts the connection to owner-managed on success. Key retain/replace/remove remains independent. A provider or destination change cannot reuse a prior key without explicit replacement/removal; only the first host-managed replacement may retain a proven absent-key state. Display-name-only edits of ordinary managed connections preserve connection revision and test validity; effective provider/URL/key changes invalidate the prior revision.
...
**Latest amendment:** Technical Specification 0.2.6 / ADR-033 approved 2026-09-13. The owner explicitly required an authenticated admin page reload to keep using its still-valid session rather than demanding another launcher grant. The strict same-origin resume endpoint reissues only session-bound CSRF/expiry metadata, uses no browser storage or grant replay, and preserves expiry/revocation/host/origin controls. The 0.2.5 connection-rebinding amendment remains in force. Prior 0.2.3 model-first implementation and full acceptance evidence remain pending. Local GGUF/Hugging Face runtime and Figma are outside this package.
...
For FR-038/040 and AC-054/055/057, chat probes use the selected provider's configured `max_output_tokens` capability instead of a separate 32-token constant. They still make one bounded call with the existing 10-second timeout, require the expected JSON result, and never activate, retry, or switch providers implicitly. This does not change the environment configuration schema or its limits.

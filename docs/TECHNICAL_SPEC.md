...
- Connection writes accept `endpoint_action: replace|retain` (default `replace` for existing clients). Creation requires `replace` and a valid `base_url`. Update with `retain` requires an omitted/null URL and uses the protected current endpoint without returning it; `replace` requires a new URL. Key retain/replace/remove remains independent. A provider or destination change cannot reuse the prior key without explicit replacement/removal. Display-name-only edits preserve connection revision and test validity; effective provider/URL/key changes invalidate the prior revision.

...
| AI 主流程 | 內容與主要操作 | 出口 |
| --- | --- | --- |
| 1. 設定 AI 模型 | 先「分析與回答模型」，再「資料查找模型」；選服務、填 API／需要時填 key／填模型名、測試、確認使用。 | 兩種角色皆有通過測試且明確選用的 revision；可改走手動。 |
...
- 資料查找模型（技術名稱：embedding model）：把程式碼整理成可查找的資料，協助找到回答依據。
...
| 只有 Chat 成功 | 明確指出資料查找模型還缺，提供該表單；AI 尚不可分析，手動仍可用。 |
...
- 失敗不只給 `MODEL_UNAVAILABLE` 或「重新檢查」。使用角色與原因導向的文字，例如「資料查找模型尚未設定」／「已選模型目前無法連線」，旁邊直接提供設定／測試／重試與手動繼續。

...
**Status:** Security contract aligned with approved Technical Specification 0.2.6; model connection code exists, model-first analysis-selection integration and evidence remain pending
...
- Failed batch items may additionally retain one application-owned reason from the closed set `NO_ELIGIBLE_CONTENT`, `PROVIDER_OUTPUT_SCHEMA_INVALID`, `PROVIDER_EVIDENCE_ID_INVALID`, or `PROVIDER_PERSONAL_INFERENCE_REJECTED`. These values interpolate no provider text or repository data. Unknown reasons are dropped before persistence; the authenticated no-store snapshot and UI expose only allowlisted code/reason values and fixed bilingual explanations.

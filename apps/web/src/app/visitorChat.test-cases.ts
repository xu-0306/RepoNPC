...
async function rejects(promise: Promise<unknown>, reason: string): Promise<void> {
  try {
    await promise;
  } catch (error) {
...
export function registerVisitorChatTests(test: RegisterTest): void {
  ...
  test("transport cancellation rejects without admitting the partial response", async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      pull(controller) {
        if (reads++ === 0) {
          controller.enqueue(encoder.encode(event("token", { delta: "partial" })));
        } else {
  ...
  test("empty or metadata-only completion does not create a successful exchange", async () => {
    await rejects(
      collectValidatedReply(streamOf(event("complete", { finish_reason: "stop" }))),
      "incomplete",
    );
  });
  ...
  test("non-string token payloads are rejected", async () => {
    await rejects(
      collectValidatedReply(streamOf(event("token", { delta: { text: "bad" } }))),
      "invalid chat token",
    );
  });
  ...
  test("normal completion releases the reader without cancelling exhausted transport", async () => {
    const stream = streamOf(event("token", { delta: "answer" }), event("complete", {}));

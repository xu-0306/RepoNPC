...
export function AdminWorkspace({
  ...
  const saveDisabled =
    !authenticated || busy || hasConflict || validation?.valid !== true;
  const profile = profileForLocale(preview?.profile, locale);
  const snippetValue = snippetText(snippet);
  const dispatchDisabled = !authenticated || busy;
  ...
  return (
    <main aria-busy={busy} className="admin-workspace" lang={locale}>
      ...
      <section aria-labelledby="admin-validation-heading">
        ...
        {warnings.length > 0 && (
          <IssueList heading={copy.warnings} issues={warnings} role="status" />
        )}
        {validation &&
          errors.length === 0 &&
          warnings.length === 0 &&
      ...
      <section aria-labelledby="admin-status-heading">
        ...
        <button disabled={dispatchDisabled} onClick={onDispatch} type="button">
          {copy.dispatch}
        </button>
      </section>
